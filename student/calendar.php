...
// Fetch confirmed and pending bookings for this student (using standard SQL date range for MySQL and SQLite compatibility)
$stmtBk = $pdo->prepare("
    ...
    WHERE b.student_id = ? AND b.booking_date >= ? AND b.booking_date <= ?
    ORDER BY b.booking_date ASC, b.start_time ASC
");
$stmtBk->execute([$student['id'], $startDate, $endDate]);
...
$pageTitle = "My Calendar Schedule";
        ...
        <div class="d-flex flex-wrap align-items-center justify-content-between mb-4">
            <div>
                <span class="pill-badge pill-badge-primary mb-2">
                    <i class="fa-regular fa-calendar me-1"></i> Interactive Schedule
                </span>
                <h2 class="fw-extrabold text-dark mb-1">My Tutoring Calendar</h2>
                <p class="text-muted mb-0">Track upcoming tutoring sessions, confirmed schedules, and lesson times.</p>
            </div>
                ...
                for ($d = 1; $d <= $daysInMonth; $d++) {
                    ...
                    if (isset($eventsByDate[$currentDateStr])) {
                        foreach ($eventsByDate[$currentDateStr] as $event) {
                            ...
                            echo '<div class="calendar-event ' . $badgeClass . '" title="' . htmlspecialchars($event['tutor_name'] . ' - ' . $event['subject_name']) . '">';
                            echo '<strong>' . $timeStr . '</strong> ' . htmlspecialchars($event['subject_name']);
                            echo '</div>';
                        }
                    }
